...
export default function CreateCabinModal({ onClose }) {
  ...
  const { isCreating, mutateCreatingCabin, error } = useCreateCabin();
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-10 w-screen overflow-y-auto bg-black/80 flex justify-center items-center flex-col "
        onClick={() => onClose()}
      >
        <div className=" w-[400px] ">
          <form
            onSubmit={handleSubmit(onSubmit, onError)}
            className={` rounded-lg shadow  p-5 border bg-white "}`}
            onClick={(e) => {
              e.stopPropagation();
            }}
          >
            ...
            {/* --------------------- Spinner Loading ---------------------*/}
            <div>{isCreating && <SmallSpinner />}</div>
            {/* --------------------- Spinner Loading ---------------------*/}
